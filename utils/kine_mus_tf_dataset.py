...
def get_windowed_dataset(dataset_path, target_size, window_size, target_joints, shift_periods=1, subjects=SUBJECTS,
    ...
    for subject in subjects:
        ...
        for adl in adl_ids:
            ...
            df[joint_delta_names] = pd.DataFrame(shifted_df[joint_names].values - df[joint_names].values,
                                                 index=df.index)
            df.drop(df.tail(shift_periods).index, inplace=True)

            #
            target_deltas = [d + "_dt" for d in target_joints]
            df = df[target_deltas + sEMG_names]
            # df = df[target_joints + sEMG_names]
            # Configure TF dataset
            tmp_dataset = tf.data.Dataset.from_tensor_slices(df.values)
            tmp_dataset = tmp_dataset.window(window_size + target_size, shift=shift_periods, stride=1, drop_remainder=True)
