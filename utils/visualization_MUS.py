...
from utils.data_loader_kine_mus import ExperimentFields
from utils.data_loader_kine_mus import RightHand
from utils.data_loader_kine_mus import sEMG
...
def plot_task_data(df, block=False):
    ...
    if not block:
        plt.ion()
    adl = numpy.unique(df[ExperimentFields.adl_id.value].values)
    ...
    ax2.plot(df[ExperimentFields.time.value], df[[e.value for e in sEMG]], '-')
    ax2.set_title('sEMG')
    ax2.set_ylabel('Electrode amplified voltage')
    ax2.set_xlabel('Time [s]')
    ax2.grid()
    ax2.legend([e.value for e in sEMG])
